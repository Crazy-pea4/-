    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <!-- 
            对于此处没有加“|| [{}]”时，控制台报错的详解
            若此时网速较慢，skuInfo先返回的是空对象，那么skuInfo.skuImageList就是undefined
            而在组件Zoom处语句skuImageList[0].imgUrl就会报错(undefined.image)
            所以利用undefined的false语义，给定一个后置条件 [{}]，保证skuIamgeList数组里面至少有一个空对象
             -->
...
export default {
  ...
  methods: {
    ...
    addToShopCart() {
      ...
      // 在工作中，会经常无法确定返回的Promise是否不报错，让我们去store中detail里看看updateShopCart方法是如何解决这一问题的
      // 相信你已经看完了在store中对该方法的解释。
      // 另一种方法也行：就是对addToShopCart进行async处理，然后对await进行try catch处理同样也能达到纠错行为，但本人跟倾向于在store中解决，这样使代码更简洁、清爽
      //  async addToShopCart() {
      //    try {
      //      await this.$store.dispatch("detail/updateShopCart", { skuId, skuNum })
