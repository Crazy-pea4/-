<template>
    <!-- 搜索框 -->
    <div class="2xl:w-2/5 xl:w-2/5 lg:w-2/5 p-3 fixed z-20 left-1/2 -translate-x-1/2 hover:-translate-y-0 transition-all duration-300"
...
const search = async () => {
    if (searchInpVal.value) {
        ...
        router.replace({ name: "Search", query: { k: searchInpVal.value } });
        searchInpVal.value = "";
    } else {
