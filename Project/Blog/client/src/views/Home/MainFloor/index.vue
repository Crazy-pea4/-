<template>
    ...
    <div class="w-full h-auto float-left" @click="ToQuestion">
        ...
        <template v-if="questionList.length > 0">
            <TransitionGroup name="questionList"
                ...
                <div class="w-full h-36 border-2 relative rounded-sm hover:shadow transition-all duration-200 flex flex-col justify-around text-center cursor-pointer my-5 px-5 first:mt-0 last:mb-0"
                    ...
                    <!-- 问题标题 -->
                    <div class="text-3xl mt-2 truncate" :data-questionId="item._id" :data-index="index">{{ item.title }}
                    </div>
                    <!-- 问题描述 -->
        ...
        <template v-else>
            <div class="absolute top-1/4 left-1/2 -translate-x-1/2 text-gray-400">
                点击旁边的新增按钮创建第一条问题吧！
            </div>
        </template>
    </div>
