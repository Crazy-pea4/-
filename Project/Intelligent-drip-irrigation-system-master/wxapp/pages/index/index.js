...
Page({
  ...
  onAutoWater(event) {
    ...
    if (sw) {
      _this.data.client.publish(
        "/hgstGAqF7eN/xiaochengxu/user/update",
        '{"params":{"autotap":1},"params2":{"autotap":1}}',
        { qos: 1 },
        function (err) {
          if (!err) {
      ...
      _this.data.client.publish(
        "/hgstGAqF7eN/xiaochengxu/user/update",
        '{"params":{"autotap":0},"params2":{"autotap":0}}',
        { qos: 1 },
        function (err) {
          if (!err) {
  ...
  doConnect() {
    ...
    _this.setData({
      client: mqtt.connect(
        "wxs://hgstGAqF7eN.iot-as-mqtt.cn-shanghai.aliyuncs.com",
        options
      ),
    });
    ...
    _this.data.client.on("connect", function () {
      ...
      _this.data.client.subscribe(
        "/hgstGAqF7eN/xiaochengxu/user/get",
        function (err) {
          if (!err) {
            console.log("订阅成功！");
          }
    ...
    _this.data.client.on("message", function (topic, message) {
      ...
      _this.setData({
        ...
        SoilMoisture: msgData.SoilMoisture,
        SoilMoisture2: msgData.SoilMoisture2,
        Distance: msgData.Distance,
        WaterOutletSwitch: msgData.WaterOutletSwitch,
        WaterOutletSwitch2: msgData.WaterOutletSwitch2,
        Tap: msgData.Tap,
        LightSwitch: msgData.LightSwitch,
        LightLux: msgData.LightLux,
        LightLux2: msgData.LightLux2,
        autolight: msgData.autolight,
        autotap: msgData.autotap,
      });
      if (msgData.Distance < 20) {
        _this.setData({
          color_d: "#d81e06",
